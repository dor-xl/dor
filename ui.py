    ...
    if RICH_OK:
        ...
        if benefits and isinstance(benefits, list) and len(benefits) > 0:
            ...
            for benefit in benefits:
                bname = benefit.get('name','')
                btotal = benefit.get('total',0)
                if "Call" in bname:
                    total = f"{btotal/60:.0f} menit"
                else:
                    quota = int(btotal)
                    if quota >= 1_000_000_000:
                        ...
                        total = str(quota)
                benefit_table.add_row(bname, total)
